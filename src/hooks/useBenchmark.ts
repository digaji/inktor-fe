...
const useBenchmark = ({ client, context }: useBenchmark) => {
  const clearCanvas = () => {
    ...
    console.log('Clear canvas start')

    for (const element of objects) {
      client.removeObject(element.id)
    }
    console.log('Clear canvas end')
  }
  ...
  const benchmarkPreset = (size: BenchmarkPresetType) => {
    ...
    for (const [key, value] of Object.entries(selectedPreset.creation)) {
      ...
      if (key === 'rectangle') {
        for (let index = 0; index < value; index++) {
          client.addRectangle(undefined, { pos: { x: x, y: y }, height: 50, width: 50 })
          x += 75
        }
      }
    ...
    for (const procedure of selectedPreset.manipulation) {
      ...
      if (procedure.element === BenchmarkElementType.CIRCLE) {
      ...
      if (procedure.element === BenchmarkElementType.RECTANGLE) {
      ...
      if (procedure.element === BenchmarkElementType.PATH) {
    ...
    console.log(`resultTime: ${resultTime} ms`)
  }
